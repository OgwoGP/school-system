...
Public Class frmFullDueList_Students
    Dim st2 As String
    Sub fillSession()
        Try
    ...
    Private Sub Button1_Click(sender As System.Object, e As System.EventArgs) Handles Button1.Click
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT RTRIM(Student.AdmissionNo),RTRIM(GRNo),RTRIM(StudentName),RTRIM(SchoolName),Sum(CourseFee.Fee) from Student,SchoolInfo,CourseFee,Class where Student.SchoolID=SchoolInfo.S_ID and Class.Classname=CourseFee.Class and CourseFee.SchoolID=SchoolInfo.S_ID and Student.Session=@d1 and CourseFee.Class=@d2  group by Student.AdmissionNo,GRNo,StudentName,SchoolName Except SELECT RTRIM(Student.AdmissionNo),RTRIM(GRNo),RTRIM(StudentName),RTRIM(SchoolName),Sum(CourseFeePayment_Join.Fee) from Student,SchoolInfo,CourseFeePayment,Class,CourseFeePayment_Join where CourseFeePayment.CFP_ID=CourseFeePayment_Join.C_PaymentID and Student.SchoolID=SchoolInfo.S_ID and Student.AdmissionNo=CourseFeePayment.AdmissionNo and CourseFeePayment.Session=@d1 and Student_Class=@d2 group by Student.AdmissionNo,GRNo,StudentName,SchoolName order by 3", con)
    ...
    Private Sub Button7_Click(sender As System.Object, e As System.EventArgs) Handles Button7.Click
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT RTRIM(Student.AdmissionNo),RTRIM(GRNo),RTRIM(StudentName),RTRIM(HostelName),RTRIM(SchoolName),SUM(Charges) FROM Student,Hosteler,Installment_Hostel,schoolInfo,HostelInfo,Class,Section where HostelInfo.HI_ID=Installment_Hostel.HostelID and Student.AdmissionNo=Hosteler.AdmissionNo and HostelInfo.HI_ID=Hosteler.HostelID and Student.SchoolID=SchoolInfo.S_ID and Class.Classname=Section.Class and Section.ID=Student.SectionID and Student.Session=@d1 and Class.ClassName=@d2 and Installment_Hostel.Installment=@d3 group by student.admissionno,GRNO,StudentName,SchoolName,HostelName Except SELECT RTRIM(Student.AdmissionNo),RTRIM(GRNo),RTRIM(StudentName),RTRIM(HostelName),RTRIM(SchoolName),SUM(Charges) FROM Student,Hosteler,Installment_Hostel,schoolInfo,HostelInfo,Class,Section,HostelFeePayment where Hosteler.HostelID=HostelInfo.HI_ID and Student.AdmissionNo=Hosteler.AdmissionNo and Installment_Hostel.HostelID=HostelInfo.HI_ID and Student.SchoolID=SchoolInfo.S_ID and Class.Classname=Section.Class and Section.ID=Student.SectionID and HostelFeePayment.HostelerID=Hosteler.H_Id and HostelFeePayment.Installment=Installment_Hostel.Installment and HostelFeePayment.Session=@d1 and HostelFeePayment.Student_Class=@d2 and Installment_Hostel.Installment=@d3 group by student.admissionno,GRNO,StudentName,SchoolName,HostelName order by 3", con)
    ...
    Private Sub Button3_Click(sender As System.Object, e As System.EventArgs) Handles Button3.Click
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT RTRIM(Student.AdmissionNo),RTRIM(GRNo),RTRIM(StudentName),RTRIM(SchoolName),Sum(CourseFee.Fee) from Student,SchoolInfo,CourseFee,Class where Student.SchoolID=SchoolInfo.S_ID and Class.Classname=CourseFee.Class and CourseFee.SchoolID=SchoolInfo.S_ID and Student.Session=@d1 and CourseFee.Class=@d2  group by Student.AdmissionNo,GRNo,StudentName,SchoolName Except SELECT RTRIM(Student.AdmissionNo),RTRIM(GRNo),RTRIM(StudentName),RTRIM(SchoolName),Sum(CourseFee.Fee) from Student,SchoolInfo,CourseFee,CourseFeePayment,Class,CourseFeePayment_Join where CourseFeePayment.CFP_ID=CourseFeePayment_Join.C_PaymentID and CourseFee.SchoolID=SchoolInfo.S_ID and Class.Classname=CourseFee.Class and Student.SchoolID=SchoolInfo.S_ID and CourseFeePayment.Student_Class=CourseFee.Class and Student.AdmissionNo=CourseFeePayment.AdmissionNo  and CourseFeePayment.Session=@d1 and CourseFee.Class=@d2  group by Student.AdmissionNo,GRNo,StudentName,SchoolName order by 3", con)
            cmd.Parameters.AddWithValue("@d1", cmbSession.Text)
            cmd.Parameters.AddWithValue("@d2", cmbClass.Text)
            ...
            If CheckForInternetConnection() = True Then
                ...
                If rdr1.Read() Then
                    For Each row As DataGridViewRow In dgw.Rows
                        ...
                        st2 = rdr1.GetValue(0)
                        Dim st3 As String = "Hello Sir/Madam, the due amount of Class Fee of the Student " & row.Cells(2).Value & " is " & row.Cells(4).Value & ""
