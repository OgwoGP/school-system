...
Public Class frmClassFeeInfo
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Try
            con = New SqlConnection(cs)
            con.Open()
            Dim ct As String = "select CourseFee.Class,Feename,SchoolID from CourseFee where Class=@d1 and Feename=@d2 and SchoolID=" & txtSchoolID.Text & ""
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "insert into CourseFee(C_ID,Class,Feename,Fee,SchoolID) VALUES (" & txtID.Text & ",@d1,@d2,@d4," & txtSchoolID.Text & ")"
            ...
            cmd.ExecuteReader()
            con.Close()
            LogFunc(lblUser.Text, "added the new Course Fee having Fee Name '" & cmbFeeName.Text & "' for Class '" & cmbClass.Text & "' and School '" & cmbSchoolName.Text & "'")
    ...
    Private Sub DeleteRecord()
        ...
        Try
            ...
            If RowsAffected > 0 Then
                LogFunc(lblUser.Text, "deleted the Course Fee having Fee Name '" & cmbFeeName.Text & "' for Class '" & cmbClass.Text & "' and School '" & cmbSchoolName.Text & "'")
    ...
    Private Sub btnUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUpdate.Click
        ...
        Try
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "Update CourseFee set Class=@d1,Feename=@d2,Fee=@d4,SchoolID=" & txtSchoolID.Text & " where C_ID=" & txtID.Text & ""
            cmd = New SqlCommand(cb)
            cmd.Connection = con
            cmd.Parameters.AddWithValue("@d1", cmbClass.Text)
            cmd.Parameters.AddWithValue("@d2", cmbFeeName.Text)
            ...
            cmd.ExecuteReader()
            con.Close()
            LogFunc(lblUser.Text, "updated the Course Fee having Fee Name '" & cmbFeeName.Text & "' for Class '" & cmbClass.Text & "' and School '" & cmbSchoolName.Text & "'")
    ...
    Public Sub Getdata()
        Try
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT RTRIM(C_ID), RTRIM(CourseFee.Class), RTRIM(Feename),RTRIM(SchoolID),RTRIM(SchoolName),RTRIM(Fee) from CourseFee,SchoolInfo where SchoolInfo.S_ID=CourseFee.SchoolID order by CourseFee.Class,FeeName", con)
    ...
    Private Sub txtSearchByClass_TextChanged(sender As System.Object, e As System.EventArgs) Handles txtSearchByClass.TextChanged
        Try
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT RTRIM(C_ID), RTRIM(CourseFee.Class), RTRIM(Feename),RTRIM(SchoolID),RTRIM(SchoolName),RTRIM(Fee) from CourseFee,SchoolInfo where SchoolInfo.S_ID=CourseFee.SchoolID and CourseFee.Class like '%" & txtSearchByClass.Text & "%' order by CourseFee.Class,FeeName", con)
            rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            dgw.Rows.Clear()
            While (rdr.Read() = True)
                dgw.Rows.Add(rdr(0), rdr(1), rdr(2), rdr(3), rdr(4), rdr(5))
    ...
    Private Sub txtSearchByFeeName_TextChanged(sender As System.Object, e As System.EventArgs) Handles txtSearchByFeeName.TextChanged
        Try
            con = New SqlConnection(cs)
            con.Open()
            cmd = New SqlCommand("SELECT RTRIM(C_ID), RTRIM(CourseFee.Class), RTRIM(Feename),RTRIM(SchoolID),RTRIM(SchoolName),RTRIM(Fee) from CourseFee,SchoolInfo where SchoolInfo.S_ID=CourseFee.SchoolID and CourseFee.FeeName like '%" & txtSearchByFeeName.Text & "%' order by CourseFee.Class,FeeName", con)
            rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            dgw.Rows.Clear()
            While (rdr.Read() = True)
                dgw.Rows.Add(rdr(0), rdr(1), rdr(2), rdr(3), rdr(4), rdr(5))
